export default {
	validateMaxEvaluators: (amount) => {
			const currentSelectedEvaluators = appsmith.store.selectedMyEmployeeDetail.length;
		...
		validateNoEvaluators: () => {
			if(appsmith.store.selectedMyEmployeeDetail.length === 0){
				return true;
			}else{
				return false;
			}
		...
		validateMinEvaluators: () => {
			if(appsmith.store.selectedMyEmployeeDetail.length < 2){
				return true;
			}else{
				return false;
			}
