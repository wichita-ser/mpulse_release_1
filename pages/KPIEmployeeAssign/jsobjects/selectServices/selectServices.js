export default {
	...
	updateSelectOption: (currentSelected) => {
		...
		get_my_team.data.forEach(item => {//can not add the same employee (check new added employee with existing one)
			const isAlreadyPresent = currentSelected.some(existingEmployee => existingEmployee["Employee ID"] === item.id);
			if (!isAlreadyPresent) {
	...
	checkAutoEvaluateEdit: () => {
		const checkIsAuto = get_department_kpi_list.data.filter((item) => item.department_indicators == SelectDepartmentKPIEdit.selectedOptionValue)
		if(checkIsAuto.length === 0){
			storeValue('checkAutoEvaluateEdit', true);
		} else if(checkIsAuto[0].auto_eva_from !== null){
			storeValue('checkAutoEvaluateEdit', true);
		} else {
	...
	checkSumWeight: () => {
		...
		for (let i = 0; i < appsmith.store.assignDepartmentKPIList.length; i++ ) {
			sumWeight += Number(helpers.transformeplaceWeight(appsmith.store.assignDepartmentKPIList[i].Weight))
		}
		if(sumWeight===100){
			return true;
		} else {
