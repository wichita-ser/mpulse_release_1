export default {
	initTrnsformDataKPI () {
		const dataKPI = [];
		if(getKPIByEmployeeID.data.payload.individual_kpis.length>0){
			getKPIByEmployeeID.data.payload.individual_kpis.map((item) => {
				dataKPI.push(
					{
						"id": item.kpi_evaluate_detail_id,
						"Perspective":item.perspective,
						"Individual KPI":item.individual_kpi,
						"Target": item.target,
	...
	transformDataAPI: async () => {
			const dataUpdate = appsmith.store.updateEmployeeIndividual;
			const add_kpi_list = appsmith.store.addEmployeeIndividual;
			const delete_kpi_list = appsmith.store.deleteEmployeeIndividual;
			const update_kpi_list = [];
			
			dataUpdate.map(item => {
				update_kpi_list.push({
					"individual_kpi_name": item.individual_kpi_name,
	...
	checkSumWeight: () => {
		...
		appsmith.store.assignEmployeeIndividual.map((item) => {
